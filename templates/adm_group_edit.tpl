...
<form name="edit" action="?" method="POST" enctype="multipart/form-data">
	...
	{if $data.id>0}
		<div style="display:inline-block;">
			<span class="head3">Картинка проекта (RU)</span><br />
		{if $data.image_ru==""}
			<input type="file" name="picture[ru]" class="short" value="{$data.name}">
		{else}
			<img src="../{$data.image_ru}" border="0"><br />
			<a href="?id={$data.id}&delimage=ru" onclick="return confirm('Вы действительно хотите удалить изображение?')">Удалить изображение</a>
		{/if}
		</div>

		<div style="display:inline-block;">
			<span class="head3">Картинка проекта (EN)</span><br />
		{if $data.image_en==""}
			<input type="file" name="picture[en]" class="short" value="{$data.name}">
		{else}
			<img src="../{$data.image_en}" border="0"><br />
			<a href="?id={$data.id}&delimage=en" onclick="return confirm('Вы действительно хотите удалить изображение?')">Удалить изображение</a>
		{/if}
		</div>
